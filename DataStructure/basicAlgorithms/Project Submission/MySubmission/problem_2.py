...
def rotated_array_search(input_list: list[int], number: int) -> int:
    ...
    while low <= high:
        ...
        if input_list[low] <= input_list[mid]:  # Left part is sorted
            if input_list[low] <= number < input_list[mid]:  # Target is in the left part
                high = mid - 1
            else:  # Target is in the right part
                low = mid + 1
        else:  # Right part is sorted
            if input_list[mid] < number <= input_list[high]:  # Target is in the right part
                low = mid + 1
